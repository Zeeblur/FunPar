...
\begin{document}
	...
	%TODO add diagram here to explain dlock
...
%	Construct a different formulation of ResetNumbers that connects the reset channel to the GSuccessor process instead of GPrefix.  You will have to write a ResetSuccssor process.  
	...
	%TODO diagram for reset numbers to ResetSuccessor
	...
	%No, the same problem persists as it is still possible to inject more values than the system can handle.	Without the reading and removing the integers from the channels as needed, there is no way to force the
